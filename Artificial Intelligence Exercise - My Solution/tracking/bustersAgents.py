...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        returnedMove = Directions.STOP
        closestDistance = 1000000
        for move in legal:
            whichGhostToTestOn = 0
            successorPosition = Actions.getSuccessor(pacmanPosition, move)
            while whichGhostToTestOn < len(livingGhostPositionDistributions):


                #if livingGhosts[whichGhostToTestOn]:
                highestProbability = 0
                locationOfGhost = None
                for i in livingGhostPositionDistributions[whichGhostToTestOn]:
                    if livingGhostPositionDistributions[whichGhostToTestOn][i] > highestProbability:
                        highestProbability = livingGhostPositionDistributions[whichGhostToTestOn][i]
                        locationOfGhost = i
                totalDistance = self.distancer.getDistance(successorPosition, locationOfGhost)
                if totalDistance < closestDistance:
                    closestDistance = totalDistance
                    returnedMove = move


                whichGhostToTestOn += 1
        return returnedMove